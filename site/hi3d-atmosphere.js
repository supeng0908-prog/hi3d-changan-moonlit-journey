...
export function atmosphere(scene,env,plan){
 const animate=[],lights=[];const cloth=new THREE.MeshStandardMaterial({color:0x8a3d30,roughness:.95,side:THREE.DoubleSide}),gold=new THREE.MeshStandardMaterial({color:0x9f824c,metalness:.5,roughness:.45});
 ...
 env.traverse(o=>{if(o.isMesh)for(const m of Array.isArray(o.material)?o.material:[o.material])if(m.userData.shader)m.userData.shader.uniforms.lakeTime.value=time;});
 };
}
